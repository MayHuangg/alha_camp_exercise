...
axios.get(INDEX_URL)
  .then(res => {
    ...
    colleagues = JSON.parse(localStorage.getItem(groupName.innerText))
    showEachPage (1,colleagues)
    makePages(colleagues)
  })
...
function searchUser (event) {
  ...
  let searchedList = colleagues.filter(user => user.name.toLowerCase().includes(searchBarValue))
